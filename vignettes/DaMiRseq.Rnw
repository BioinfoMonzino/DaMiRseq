%\VignetteIndexEntry{Comprehensive Data Mining Pipeline for RNA-seq data: normalization, features selection and classification - DaMiRseq package}
...
\title{The DaMiRseq package - A Data Mining pipeline for RNA-seq data: normalization, feature selection and classification}
...
\damir{} package needs two files as input data: 1) raw counts table and 2) class and (if present) covariates information table. In this vignette,
we will use the dataset described in Section~\ref{data_prep}; we can store the path to the count and covariate files into objects as follows:
...
Then, we import the files by \Rfunction{DaMiR.read} function:

<<chu_2>>=
set.seed(123456789)
list.res <- DaMiR.read(filecounts, filecovariates)
dds <- list.res$dds
df_clcov <- list.res$classCov
@
Data are stored in a \Robject{list}, which contains two objects: a \Rclass{DESeqDataSet} object that stores expression data \footnote{See \Biocpkg{DESeq2} \cite{love2014moderated} Vignette, for more details.} and a \Robject{data frame} of covariates.
\textbf{Note.} We have already used the \Rfunction{set.seed()} function that allows the user to make the results of the whole pipeline reproducible.

Data can be inspected by \Rfunction{head} function:
<<chu_3>>=
head(counts(dds)[,c(1:5,85:89)])
head(df_clcov[c(1:5,85:89),], n = 10)
@
\textbf{Note.} The \textit{``df\_clcov''} data frame, containing the covariates information, includes also the   \textbf{ \textit{'class'}} column as it is required by the instructions (mandatory).
...
allows the removal of inconsistent samples.
  ...
  <<chu_4>>=
  data_norm <- DaMiR.normalization(dds, minCounts=10, fSample=0.7,
                                   hyper = "no")
  head(data_norm[,c(1:5,85:89)])
  head(df_clcov[c(1:5,85:89),], n=10)
  @
  In this case, 19604 genes with read counts greater than 10 (\Rcode{minCounts = 10}) in at least 70\% of samples (\Rcode{fSample = 0.7}), have been selected, while 36714 have been filtered out. The dataset, consisting now of 19604 genes, is then normalized by the \Rfunction{varianceStabilizingTransformation} function
  of the \Biocpkg{DESeq2} package \cite{love2014moderated}. Printing a few lines by the \Rfunction{head()} function, we can see that ``VST'' transformation produces data on the log2 scale normalized with respect to the library size.
...
\textbf{Note.} Computing a 'class' restricted CV may prevent the removal of features that may be specifically associated with a certain class. This could be important in some biological contexts, for example, for immune genes whose expression under definite conditions may unveil peculiar class-gene associations. \\
  ...
  head(data_norm[,c(1:5,85:89)])
  @
  The \Rcode{th.cv = 3} allows the removal of a further 49 ``hypervariant'' genes from the gene expression data matrix. The number of genes is now reduced to 19555.
  ...
  <<chu_7>>=
  list.res2 <- DaMiR.sampleFilt(data_norm, df_clcov, th.corr=0.93)
  data_filt <- list.res2$data
  df_clcov <- list.res2$classCov
  head(data_filt[,c(1:5,85:89)])
  @
Fourteen samples were discarded because their mean absolute correlation is lower than 0.93. Filtered data was stored in a \Robject{list}, which contains two objects: a  \Rclass{matrix} of normalized expression values and a \Robject{data frame} of covariates information.
...
After data normalization, we propose to test for the presence of surrogate variables (sv) in order to remove the effect of putative confounding factors from the expression data. Nonetheless, the algorithm cannot distinguish among real technical batches and important biological effects (such as environmental, genetic or demographic variables) whose correction is not desirable. Therefore we enable the user to evaluate whether any of the retrieved sv is correlated or not with one or more known variables. Thus, this step gives the user the opportunity to choose the most appropriate number of sv to be used for expression data adjustment \cite{leek2007capturing, jaffe2015practical}.
  ...
  <<chu_8, dev="pdf">>=
   sv <- DaMiR.SV(data_filt, df_clcov)
  @
  Using default values (\Rcode{"fve"} method and \Rcode{th.fve = 0.95}), we obtained a matrix with 7 sv that is the number of sv which returns ~95\% of variance explained. Figure~\ref{fig_fve} shows all the sv computed by the algorithm with respect to the corresponding fraction of variance explained.
  ...
  \begin{figure}[!htbp]
    ...
    the proportion of variance, explained by a sv together with the prior ones. The red dot marks the upper limit of sv that should be used to adjust the data. Here, 7 is the maximum number of sv obtained as it corresponds to 95\% of variance explained.}
    \label{fig_fve}
  \end{figure}
  \FloatBarrier
  ...
  wish to correct. For this purpose, we correlate the sv with the known variables stored in the ``df\_clcov'' data frame in order to decide if all of these sv (or only a subset of them) may be used to
  adjust the data.
  <<chu_9, dev="pdf">>=
   DaMiR.corrplot(sv, df_clcov)
  @
  The \Rfunction{DaMiR.corrplot} function produces a correlation plot where significant correlations (here the threshold is set to the default
  \Rcode{sig.level = 10\textsuperscript{-4}}) are shown within colored circles (blue or red gradient). In Figure~\ref{fig_corr}, we can see that the first two sv significantly correlate
  with the ``death'' and ``center'' variables (-0.3 and 0.44 repsectively), while the third is not correlated with known covariates and presumably, recovers the effect of an unmeasured covariate; moreover, the fourth to seventh sv have significant correlation with ``sex''. Since in this vignette we are not interested in adjusting the data for such demographics/biological variables, we only consider the first 3 sv.\\
  ...
  \begin{figure}[!htbp]
    ...
    \caption{Correlation Plot between sv and known covariates. This plot highligths the correlation between sv and known covariates, using both color gradient and circle size. The color ranges from dark red (correlation = -1) to dark blue (correlation = 1) and the circle size is maximum for a correlation equal to 1 or -1 and decreases up to zero. Black crosses help to identify non significant correlations. This plot shows that the first and the second sv significantly correlate with the ``death'' and ``center'' variables (-0.3 and 0.44 respectively), while the third is not correlated with known covariates and presumably, recovers the effect of a missed covariate; moreover, fourth to seventh sv significantly correlate with ``sex''.}
    \label{fig_corr}
  \end{figure}
  \FloatBarrier
...
\end{description}
In this vignette, \Rfunction{DaMiR.Allplot} is used to appreciate the effect of data adjusting (see Section~\ref{adj_data}). First, we check how data appear just after normalization: the heatmap in Figure~\ref{fig_n1} and MDS plots in Figures~\ref{fig_n3} and ~\ref{fig_n4} highlight the presence of specific clusters, mostly due to \textit{'center'}, \textit{'death'} covariates whereas the two \textit{'classes'} overlie.
\textbf{Note.} If a covariate contains missing data (i.e. ``NA'' values), the function cannot draw the plot that bear covariate information. The user is, however, encouraged to impute missing data if s/he considers it meaningful to plot the covariate of interest for ``diagnosis'' purposes.
  ...
  DaMiR.Allplot(data_norm, list.res$classCov)
  @
...
\newpage
After removing noise effets from our expression data, as discussed in Section~\ref{adj_data}, we may appreciate the result of sv adjustiment on the data: now, the heatmap in Figure~\ref{fig_n5} and MDS plots in Figures~\ref{fig_n7} and ~\ref{fig_n8} do not exhibit any specific clusters, except those related to \textit{'class'}. Moreover, the effect on data distribution is irrelevant: both RLE in Figures~\ref{fig_n1} and ~\ref{fig_n5} show minimal shifts from the zero line.

  <<chu_12, dev="pdf">>=
  # After outliers removing and SV adjusting
  DaMiR.Allplot(list.res3$data,list.res3$classCov)
  @
...
\textbf{Note.} Before running the \Rfunction{DaMiR.FSelect} function, we need to transpose our normalized expression data. Actually, the function \Rfunction{DaMiR.transpose} transposes data but also tries to prevent the use of tricky feature labels. The ``-'' and ``.'' characters within variable labels (commonly found, for example, in gene symbols) may, in fact, cause errors if included in the model design as it is required to execute part of the code of the \Rfunction{DaMiR.FSelect} function. Thus, we, firstly, search and, eventually, replace them with non causing error characters.
 <<chu_13>>=
 data_clean<-DaMiR.transpose(list.res3$data)
 list.res4 <- DaMiR.FSelect(data_clean, df_clcov)
 # head(list.res$data[, c(1:5, 85:89)])
 @
The list.res4 object returns a reduced expression matrix with important features. As we can see, the initial number of 19555 features has been reduced to 306.
...
Some of the returned important features may, however, be highly correlated. To prevent the inclusion of redundant features that may decrease the model performance during the classification step, we apply a function that produces a pair-wise absolute correlation matrix. When two features present a correlation higher than \Rcode{th.corr} argument, the algorithm calculates the mean absolute correlation of each feature and, then, removes the feature with the largest mean absolute correlation.
 <<chu_14, dev="pdf">>=
  data_clean_red <- list.res4$data
  data_clean_red <- DaMiR.FReduct(data_clean_red)
  DaMiR.MDSplot(data_clean_red, df_clcov)
  @
In our example, we used a Spearman's correlation metric and a correletion threshold of 0.85 (default). This reduction step filters out 39 highly correlated genes over the 306 returned by the \Rfunction{DaMiR.FSelect}. The figure below shows the MDS plot drawn by the use of the expression matrix of the remaining 267 genes.
\begin{figure}[!htbp]
...
<<chu_15, dev="pdf">>=
  ...
  df.importance <- DaMiR.FSort(data_clean_red, df_clcov)
  head(df.importance)
@
...
<<chu_16, dev="pdf">>=
  ...
  list.res5 <- DaMiR.FBest(data_clean_red, df.importance, n.pred = 5)
  data_for_class <- list.res5$data

  # Dendrogram and heatmap
  DaMiR.Clustplot(data_for_class,df_clcov)
  @
...
In this case, before implementing the classification procedure, we created a ``class'' vector. To speed up the execution time of the function, we set \Rcode{iter = 50} (default is 100) even though we suggest to use an higher number of iteration to obtain more accurate results.

<<chu_17, dev="pdf">>=
  classes <- df_clcov$class
  list.resFinal <- DaMiR.EnsembleLearning(data_for_class, classes, iter = 50)
@
...
As shown in Figure~\ref{fig_n10}, all weak classifiers show a very high (greater than 95\% in average) classification accuracy (RF: $95.55 \pm 2.82$, SVM: $96.3 \pm 2.02$, NB: $95.43 \pm 2.26$, LDA: $96.94 \pm 1.98$, LR: $95.47 \pm 2.64$, 3kNN: $95.4 \pm 2.5$). As discussed in Section~\ref{intro},the meta-learner is more influenced by better weak classifiers than inferior ones, which ensures ``Ensemble'' to reach a classification accuracy equal to $96.75 \pm 1.83$.
...
In this section, we highlight and discuss how variations to the \damir{} pipeline, extensively described in Section~\ref{sect2}, could affect the final classification results. Results will be discussed in Section~\ref{perf_comp}. \\
\textbf{Note.} For simplicity, here we are do not produce all plots, except for the violin plot generated by \Rfunction{DaMiR.EnsembleLearning}, used for performances comparison. However, the usage of \Rfunction{DaMiR.Allplot}, \Rfunction{DaMiR.corrplot}, \Rfunction{DaMiR.Clustplot} and \Rfunction{DaMiR.MDSplot} \textbf{is crucial} to check the effect of each process.


\subsection{Standard workflow, changing the seed} \label{std_seed}
Here, we have summarized the \damir{} standard workflow, described in Section~\ref{sect2}; we have just modified the value of the seed, setting \Rcode{set.seed(987654321)} instead of \Rcode{set.seed(123456789)}. The aim is to check the robustness of ``Ensemble'', changing training and test sets used before.\\
...
## Feature Selection
data_clean<-DaMiR.transpose(list.res2$data)
list.res4 <- DaMiR.FSelect(data_clean, df_clcov)
data_clean_red <- list.res4$data
data_clean_red <- DaMiR.FReduct(data_clean_red)
# DaMiR.MDSplot(data_clean_red, df_clcov)
...
The number of predictors obviously affects the classification accuracy: a small number of features could not be sufficient to ensure a good classification power; on the other hand, many predictors allow better classification accuracies if they are truly associated with the response, but at the cost of a possible reduction of the generalization ability. In this case, we decide to use 40 predictors instead of the 5 employed in the standard workflow.
We set again the same seed used in Section~\ref{sect2} (\Rcode{set.seed(123456789)}), which ensures correct comparisons between results.
...
# DaMiR.MDSplot(data_clean_red, df_clcov)
df.importance <- DaMiR.FSort(data_clean_red, df_clcov)
list.res5 <- DaMiR.FBest(data_clean_red, df.importance,n.pred = 40)
data_for_class <- list.res5$data
# DaMiR.Clustplot(data_for_class,df_clcov)
...
In this example, we used the data of the ``sex'' covariate stored in the second column of the ``df\_clcov'' data frame.
We set the same seed used in Section~\ref{sect2} (\Rcode{set.seed(123456789)}), which ensures correct comparisons between results.
...
## Classification
classes <- df_clcov$class
# we consider Sex as well
add.features <- df_clcov[,2,drop=FALSE]
list.resFinal <- DaMiR.EnsembleLearning(data_for_class,
                                        classes,
                                        covariates = add.features, iter = 50)
...
\begin{figure}[!htbp]
\includegraphics[width=.6\textwidth]{figure/chu_18-3} % Standard WF changing seed
\includegraphics[width=.6\textwidth]{figure/chu_19-2} % no SV
\includegraphics[width=.6\textwidth]{figure/chu_20-3} % 40 predictors
\includegraphics[width=.6\textwidth]{figure/chu_21-3} % adding SEx
\caption{Performance comparison. Each violin plot shows the effect of a specific modification to \damir{} standard workflow, described in Section~\ref{sect2}; in particular, we can see at the top left part of this figure, results of Section~\ref{std_seed} workflow (\textit{Standard Workflow, changing seed}); at the top right, results of Section~\ref{not_adj}'s workflow (\textit{Without cleaning the data}); at the bottom left, results of Section~\ref{more_pred} workflow (\textit{Changing the number of predictors}); and, at the bottom right, results of Section~\ref{add_feat} workflow (\textit{Adding heterogeneous features for classification})}.
\label{fig_n11}
\end{figure}


\begin{description}[align=left]
\item ['Standard Workflow' vs 'Standard Workflow, changing the seed'.] It is well known that, changing training and test sets, a specific classifier may not have the best performances. However, the stability and the robustness of a classifier affects the generalization capability. An advantage of adopting a Staking strategy is to be as stable and robust as possible. In this case the accuracy of both ``Ensemble'' meta-learner is quite similar, even though the performances of weak classifiers are different. This means that the choice of a single classifier, even when it shows the higher prediction accuracy, do not always ensure obtaining the best accuracy on new unseen data. The Ensemble, instead, is always close to the best prediction accuracy. The accuracy changes from  $96.75 \pm 1.83$ (Standard Workflow) to $96.72 \pm 2.25$. See Figure~\ref{fig_n10} and upper left panel in Figure~\ref{fig_n11}.

\item ['Standard Workflow' vs 'Without cleaning the data'.] Without adjusting data (following the steps described in Section~\ref{adj_data}), performances usually decrease; this could be explained by the fact that some noise, probably coming from unknown variation, is present in the dataset. In this example, accuracy decreases from $96.75 \pm 1.83$ (Standard Workflow) to $93.85 \pm 2.52$. See Figure~\ref{fig_n10} and upper right panel in Figure~\ref{fig_n11}.

\item ['Standard Workflow' vs 'Changing the number of predictors'.] It is always good practice to achieve a trade-off between model complexity, generalization capability and classification power. Increasing the number of predictors improved the performances, even if in this case the classification power does not increase drammatically; in this example, accuracy increases from $96.75 \pm 1.83$ (Standard Workflow) to $98.53 \pm 1.8$. See Figure~\ref{fig_n10} and lower left panel in Figure~\ref{fig_n11}.

\item ['Standard Workflow' vs 'Adding heterogeneous features for classification'.] We included the ``sex'' variable together with predictor genes to generate new classification models. Demographic variables may play an important role in some classification settings and may, in fact, impact prediction. However, in this case the ``sex'' variable does not impact deeply on prediction since it only slightly increases the accuracy from $96.75 \pm 1.83$ (Standard Workflow) to $97.32 \pm 1.83$. See Figure~\ref{fig_n10} and bottom lower panel in Figure~\ref{fig_n11}.
\end{description}
