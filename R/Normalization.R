...
#'
#'
DaMiR.normalization <- function(data, minCounts=10, fSample=0.5, hyper=c("yes","no"), th.cv=3, type=c("vst","rlog")){
  # check arguments
  ...
  if(!(is(data, "SummarizedExperiment"))) stop("'data' must be a 'SummarizedExperiment' object")
  ...
  data_norm<-SummarizedExperiment(assays=as.matrix(data_norm), colData=as.data.frame(colData(data)))
...
#'
#' @return A \code{SummarizedExperiment} object which contains a normalized and filtered
#' expression matrix (log2 scale) and a filtered data frame with 'class' and (optionally) covariates.
#'
#' @author Mattia Chiesa, Luca Piacentini
#'
#' @examples
...
#'
#'
DaMiR.sampleFilt <- function(data, th.corr=0.9, type=c("spearman","pearson")){
  ...
  if (!(class(data)[1]=="SummarizedExperiment")) {
    stop("'data' must be a 'SummarizedExpression' object")}
  else { count_data<-assay(data)
  }
  ...
  cat(number_of_samples-dim(data_filt)[2],"Samples have been excluded by averaged Sample-per-Sample correlation.","\n",dim(data_filt)[2], "Samples remained.","\n")
